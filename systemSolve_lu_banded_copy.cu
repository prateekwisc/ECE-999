...
//    culaDouble one = 1.0;
    ...
    printf("Allocating Matrices\n");
    A = (culaDouble*)malloc(N*nrows*sizeof(culaDouble)); 
    A_bak = (culaDouble*)malloc(N*nrows*sizeof(culaDouble));
...
//    info = (int *)malloc(N * sizeof(int));
    ...
    printf("Allocating Matrices on device\n");
    cudaMalloc((void**)&Ad,N*nrows*sizeof(culaDouble));
    cudaMalloc((void**)&Ad_bak,N*nrows*sizeof(culaDouble));
...
//Transposing the matrix A
    ...
    for(i = 0; i < N; ++i)
     ...
     // Set A to the identity matrix
    memset(A, 0 , N*nrows*sizeof(culaDouble));
    memset(A_bak, 0 , N*nrows*sizeof(culaDouble));
    for(i = 0; i < 2*k+1; ++i) {
      for (j = 0; j< N; ++j){

         A_bak[i * N + j] = A[j*N + i] = rand() %10;
      }
    }
  /*    if (i > 1){
        ...
        { for (i = 0; i < (j-k); i++)
          {
        A_bak[i * N + i-j] = A[i*N + i-j] = rand%10;       
          }
        }
        if (i < N - 1){
          for(j = 1; j<(kl + ku); j++)
...
*/
    // Set B to a random matrix (see note at top
    for(i = 0; i < N; ++i)
        B[i] = (culaDouble)(rand() % 10 + 1);
    memcpy(X, B, N*sizeof(culaDouble));

    memset(IPIV, 0, N*sizeof(culaInt));

//Copy from Host to Device
    cudaMemcpy(Ad,A, N*nrows*sizeof(culaDouble),cudaMemcpyHostToDevice);
    cudaMemcpy(Ad_bak,A_bak, N*nrows*sizeof(culaFloat),cudaMemcpyHostToDevice);
    cudaMemcpy(Bd,B, N*sizeof(culaDouble),cudaMemcpyHostToDevice);
    cudaMemcpy(Xd,X, N*sizeof(culaDouble),cudaMemcpyHostToDevice);
    cudaMemcpy(IPIVd,IPIV, N*sizeof(culaInt),cudaMemcpyHostToDevice);

    printf("Calling culaDgbtrf\n");
    int iter = 0;
    status = culaDeviceDgbtrf(N, N, k, k, Ad, nrows, IPIVd);
   // printf("iter = %d\n", iter);
...
//Copy result from Device to Host
    ...
    cudaMemcpy(A,Ad, N*nrows*sizeof(culaDouble),cudaMemcpyDeviceToHost);
    cudaMemcpy(A_bak,Ad_bak, N*nrows*sizeof(culaFloat),cudaMemcpyDeviceToHost);
    cudaMemcpy(B,Bd, N*sizeof(culaDouble),cudaMemcpyDeviceToHost);
    cudaMemcpy(X,Xd, N*sizeof(culaDouble),cudaMemcpyDeviceToHost);
    cudaMemcpy(IPIV,IPIVd, N*sizeof(culaInt),cudaMemcpyDeviceToHost);
...
//gflop = (((2/3*N*N*N) + 2*N*N)*1000)/t_dgetrf;
printf("\n Time taken for CULA Dgbtrf is %f ms \n", t_dgetrf);
gflop = ((((2/3)*N*N*N) + 2*N*N)*1000)/t_dgetrf;
//printf("\n Flop rate is: %f FLOPS/s", gflop);
